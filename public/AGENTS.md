# Agent guide — Persona personal site

You are working with a **Persona** site: Next.js App Router + a compiled LLM Wiki.

## Read first
1. `/llms.txt` — machine map of endpoints and skills
2. `/openapi.json` — stable API contract (prefer this over scraping HTML)
3. `content/persona.config.ts` — identity and module toggles
...
- After wiki edits, run `npm run persona -- compile` (or POST `/api/v1/admin/compile`) to refresh `content/wiki-graph.json`.
- Contradictions belong in wiki frontmatter / “Views that evolved” — not by rewriting sources.

## API (one service layer)
| Surface | Path |
|---|---|
| REST | `/api/v1/{persona,blog,projects,resume,wiki,search,chat}` |
